...
if (message.content.startsWith(prefix + 'credit')) {
  ...
  if(men) {
message.channel.send(`**<@!${mention.id}>, , :credit_card:  balance is: **\`${profile[men.id].credits}$``)
} else {
  message.channel.send(`**<@!${mention.id}>,, :credit_card: balance is: **\`${profile[message.author.id].credits}$``)
}
}
...
if(message.content.startsWith(prefix + "daily")) {
  if(profile[message.author.id].lastDaily != moment().format('day')) {
    ...
    message.channel.send(`**:stopwatch: | <@!${mention.id}>,, your daily :yen: credits refreshes ${moment().endOf('day').fromNow()}**`)
}
  }
...
message.channel.send(`<@!${mention.id}>, has tranfered \`$${args[1]}\` to ${men}.`)
}

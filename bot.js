...
client.on('message' , message => {
   ...
   const credits = require('./credit.json');
const creditsPath = './credit.json';
client.on('message',async message => {
    if(message.author.bot || message.channel.type === 'dm') return;
    let args = message.content.split(' ');
   ...
   if(args[0].toLowerCase() == `${prefix}credit` || args[0].toLowerCase() === `${prefix}credits`) {
       ...
       if(!credits[mention.id]) return message.channel.send(`** <@!${mention.id}>, I Can't Find it,!**`);
       if(!args[2]) {
                message.channel.send(`**<@!${mention.id}>, Your :credit_card: Balance is ${credits[mention.id].credits}$**`);
       
       } else if(mentionn && args[2]) {
if(args[2] < 1) return message.channel.send(`**<@!${mention.id}>, The "Number" You Entered Isn't Correct**.`)      
if(mentionn.id === message.author.id) return message.channel.send(`**<@!${mention.id}>, You Can't Give Credits To Yourself**.`);
           if(args[2] > credits[author].credits) return message.channel.send(`**<@!${mention.id}>, Your Balance is Not Enough For That,?!**`);
         ...
         message.channel.send(`** Type \`${num}\` To Complete The Transfer!**`).then(m => {
             message.channel.awaitMessages(r => r.author.id === message.author.id, { max: 1, time: 20000, errors:['time'] }).then(collected => {
                 let c = collected.first();
                      let mentionn = message.mentions.users.first();
                 if(c.content === num) {
                         message.channel.send(`**You Have Got Transferred ${args[2]}$ ${mentionn},!**`);
                         m.delete();
                         c.delete();
         ...
         message.channel.send(`**<@!${mention.id}>, The Syntax should be like ${prefix}credits <Mention> [Ammount]**`);
     }
 } else if(args[0].toLowerCase() === `${prefix}daily`) {
          let mention = message.mentions.users.first() || message.author;
       let mentionn = message.mentions.users.first();
   if(credits[author].daily !== 86400000 && Date.now() - credits[author].daily !== 86400000) {
         message.channel.send(`** <@!${mention.id}> , You're Already Got The Daily Gift From Credit,!**`);
     } else {
let ammount = parseInt( Math.random() * 175 + 500 );
       credits[author].daily = Date.now();
         credits[author].credits += ammount;
              let mentionn = message.mentions.users.first();
         fs.writeFileSync(creditsPath, JSON.stringify(credits, null, 4));
         message.channel.send(`** <@!${mention.id}>, You Received Your :credit_card: ${ammount}$ Daily Credits,!**`);
     }
 }
});
